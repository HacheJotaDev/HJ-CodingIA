      ...
      switch (command) {
        ...
        case "/model": {
          ...
          if (modelName && AVAILABLE_MODELS.some((m) => m.id === modelName)) {
            ...
            addMessage(
              "assistant",
              `Model switched to **${AVAILABLE_MODELS.find((m) => m.id === modelName)?.name}**.`
            );
          } else {
            addMessage(
              "assistant",
        ...
        case "/caveman":
          setSpeechMode("caveman");
          addMessage("assistant", "🦣 Caveman mode ON. Responses compressed.");
          break;
        case "/rocky":
          setSpeechMode("rocky");
          addMessage("assistant", "🪨 Rocky mode ON. Alien grammar activated.");
          break;
        case "/normal":
          setSpeechMode("normal");
          addMessage("assistant", "💬 Normal mode. Standard responses restored.");
          break;
        case "/cost":
          addMessage(
            "assistant",
        ...
        case "/export": {
          ...
          a.href = url;
          a.download = `hj-codingia-session-${Date.now()}.md`;
          a.click();
        ...
        case "/think":
          ...
          addMessage(
            "assistant",
            `🧠 Extended thinking ${thinkingEnabled ? "disabled" : "enabled"}.`
          );
          break;
        case "/review":
          addMessage(
            "assistant",
            "🔍 Review mode activated. Paste your code and I'll perform a thorough code review covering security, performance, maintainability, and best practices."
          );
          break;
        case "/plan":
          addMessage(
            "assistant",
            "📋 Planning mode activated. Describe what you want to build and I'll create a detailed implementation plan before writing any code."
          );
          break;
        case "/agents":
          addMessage(
        ...
        case "/goals":
          addMessage(
            "assistant",
            "🎯 Goal tracking is available for this session. Describe what you want to accomplish and I'll help you track progress against your objectives."
          );
          break;
        default:
          addMessage(
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-neutral-300">
                {activeSession?.title || "HJ CodingIA"}
              </span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant="outline"
              className="text-[10px] border-white/[0.08] text-neutral-500 bg-white/[0.02]"
            >
              ...
              <div className="relative z-10 flex flex-col items-center">
                <div className="relative mb-8">
                  ...
                  <img
                    ...
                    alt="HJ CodingIA"
                    className="relative w-24 h-24 animate-float drop-shadow-[0_0_30px_rgba(233,30,99,0.3)]"
                  />
                </div>
                  ...
                  {[
                    {
                      icon: "💻",
                      title: "Build an App",
                      desc: "Describe a web app you want to build",
                      prompt: "Help me build a modern web application with user authentication and a dashboard",
                    },
                    {
                      icon: "🐛",
                      title: "Debug Code",
                      desc: "Paste code that needs debugging",
                      prompt: "Help me debug this code, it's throwing an error",
                    },
                    {
                      icon: "🏗️",
                      title: "Architecture",
                      desc: "Design system architecture",
                      prompt: "Design a microservices architecture for an e-commerce application",
                    },
                    {
                      icon: "🧪",
                      title: "Write Tests",
                    ...
                    <button
                      ...
                      className="flex items-start gap-3 p-4 rounded-xl bg-white/[0.02] border border-white/[0.06] hover:border-[#e91e63]/30 hover:bg-[#e91e63]/5 transition-all text-left group border-glow"
                    >
                      ...
                      <div>
                        ...
                        <div className="text-[11px] text-neutral-600 group-hover:text-neutral-400 transition-colors mt-0.5">
                          {item.desc}
                        </div>
                      </div>
                    </button>
            ...
            <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
              {messages.map((msg, index) => (
                <div
                  ...
                  } ${messageAnimations.has(msg.id) ? "animate-slide-up" : ""}`}
                >
                  {msg.role === "assistant" && (
                  ...
                  <div
                    className={`group relative max-w-[85%] rounded-xl ${
                      msg.role === "user"
                        ? "bg-[#e91e63]/10 border border-l-2 border-l-[#e91e63]/40 border-r-white/[0.06] border-t-white/[0.06] border-b-white/[0.06] px-4 py-3"
                        : "bg-[#0a0a0a] border border-white/[0.06] px-5 py-4"
                    }`}
                  >
                    {msg.role === "user" ? (
                        ...
                        <>
                          <button
                            onClick={() => {
                              ...
                              const a = document.createElement("a");
                              a.href = url;
                              a.download = `hj-codingia-response-${msg.id}.md`;
                              a.click();
                              URL.revokeObjectURL(url);
                            }}
                            className="p-1 rounded hover:bg-white/[0.06] text-neutral-600 hover:text-neutral-300 transition-all"
            ...
            {showCommands && filteredCommands.length > 0 && (
              <div className="mb-2 rounded-lg bg-[#0a0a0a] border border-white/[0.08] overflow-hidden animate-fade-in">
                {filteredCommands.map((cmd) => (
                  ...
                  >
                    <code className="text-xs font-mono text-[#e91e63] bg-[#e91e63]/8 px-1.5 py-0.5 rounded">
                      {cmd.name}
                    </code>
                    ...
                    <Badge
                      variant="outline"
                      className="text-[9px] border-white/[0.06] text-neutral-600 ml-auto"
                    >
                      {cmd.category}
                    </Badge>
                  </button>
            ...
            >
              <div className="flex-1 relative">
                ...
                <textarea
                  ...
                  placeholder="Ask HJ CodingIA anything... (type / for commands)"
                  rows={1}
                  className="w-full resize-none rounded-xl bg-[#0a0a0a] border border-white/[0.08] focus:border-[#e91e63]/40 focus:ring-1 focus:ring-[#e91e63]/20 pl-10 pr-20 py-3 text-sm text-neutral-200 placeholder:text-neutral-600 font-mono transition-all outline-none"
                  style={{
                    minHeight: "46px",
                    maxHeight: "200px",
                    height: "auto",
              ...
              <Button
                ...
                className="bg-[#e91e63] hover:bg-[#ff2a76] text-white h-[46px] px-4 shadow-lg shadow-[#e91e63]/20"
              >
                {isLoading ? (
