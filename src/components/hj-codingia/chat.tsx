  ...
  const sendMessage = useCallback(
    async (e?: FormEvent) => {
      ...
      if (!apiKey && !modelIsFree) {
        setApiKeyWarning(`No API key configured for ${currentProvider?.name}. Go to Settings → API Keys to add your key, or switch to a Z AI model.`);
        return;
      }
      ...
      try {
        ...
        if (error instanceof Error && error.name === "AbortError") {
          ...
          addMessage("assistant", `## Error\n\n\`${errMsg}\`\n\nPlease try again. Z AI models work automatically — no API key needed.`);
        }
      } finally {
        setIsLoading(false);
        setStreamingContent("");
          ...
          {messages.length === 0 && !isLoading ? (
            <div className="h-full flex flex-col items-center justify-center px-6 relative">
              ...
              <div className="relative z-10 flex flex-col items-center">
                ...
                <h2 className="text-3xl font-bold mb-3">
                  Welcome to <span className="text-gradient">HJ CodingIA</span>
                </h2>
                <p className="text-neutral-500 text-sm text-center max-w-lg mb-10 leading-relaxed">
                  Your professional AI coding assistant. Write code, debug, refactor, plan, and ship — all in your browser. No API key needed.
                </p>

                <div className="mb-6 px-4 py-2.5 rounded-xl bg-[#e91e63]/10 border border-[#e91e63]/20 max-w-md w-full">
                  <div className="flex items-center gap-2">
                    <span className="text-sm">⚡</span>
                    <span className="text-sm font-medium text-[#f472b6]">Powered by Z AI — Free, no API key needed</span>
                  </div>
                  <p className="text-xs text-[#f472b6]/60 mt-1">
                    Works out of the box with Z AI models. Auto-fallback on Vercel. Add your own API keys in Settings for premium models.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-xl w-full">
                  {[
                    ...
                    <button
                      key={item.title}
                      onClick={() => { setInputValue(item.prompt); inputRef.current?.focus(); }}
                ...
                <p className="text-[10px] text-neutral-700 mt-8 tracking-wider uppercase">
                  Powered by Z AI • No API key needed
                </p>
              </div>
            </div>
          ) : (
            <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
              {messages.map((msg, index) => (
                <div
                  key={msg.id}
                  className={`flex gap-3 ${msg.role === "user" ? "justify-end" : ""} ${messageAnimations.has(msg.id) ? "animate-slide-up" : ""}`}
                >
                  {msg.role === "assistant" && (
                      ...
                      {msg.role === "assistant" && (
                        <>
                          <button
                            ...
                            className="p-1 rounded hover:bg-white/[0.06] text-neutral-600 hover:text-neutral-300 transition-all" title="Download"
                          >
                            <Download className="w-3 h-3" />
                          </button>
                          {index === messages.length - 1 && (
                            <button onClick={regenerateLastMessage} className="p-1 rounded hover:bg-white/[0.06] text-neutral-600 hover:text-neutral-300 transition-all" title="Regenerate">
            ...
            {showCommands && filteredCommands.length > 0 && (
              <div className="mb-2 rounded-lg bg-[#0a0a0a] border border-white/[0.08] overflow-hidden animate-fade-in">
                {filteredCommands.map((cmd) => (
                  <button
                    key={cmd.name}
                    onClick={() => { setInputValue(cmd.name + " "); setShowCommands(false); inputRef.current?.focus(); }}
            ...
            <form onSubmit={sendMessage} className="relative flex items-end gap-2">
              <div className="flex-1 relative">
                <div className="absolute left-3 bottom-3 z-10">
                  <button type="button" className="p-0.5 text-neutral-600 hover:text-neutral-400 transition-colors" title="Attach file (coming soon)">
